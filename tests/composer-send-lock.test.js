...
assert.match(composerSource, /var matchesElement = warning \? warning\.querySelector\('\.ticketmailer-mailbox-matches'\) : null;/);
assert.match(composerSource, /if \(matchesElement\) \{\s*matchesElement\.textContent/);
...
assert.match(composerSource, /currentRequest !== validation\.requestId\) \{\s*resolve\(\);\s*return;/);
